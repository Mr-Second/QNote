...
; 完成页"运行 QNote"复选框（基于默认模板追加，06-21-installer-packaging）
!define MUI_FINISHPAGE_RUN "$InstDir\QNote.exe"
!define MUI_FINISHPAGE_RUN_TEXT "$(MsgFinishRun)"
...
; ============================================================================
; Bilingual strings (06-21-installer-packaging)
; NSIS auto-selects by $LANGUAGE (system default UI language).
; Chinese strings live in packaging/zh_strings.nsh (UTF-8 BOM) because
; xpack's io.gsub mangles non-ASCII in the specfile itself.
!include "${PACKAGE_WORKDIR}\packaging\zh_strings.nsh"
...
Function .onInit
  ...
  ; 安装前检测 QNote 是否运行（06-21-installer-packaging）
  ; 覆盖安装时若 QNote 在运行，exe/DLL 被锁会失败
  check_running_install:
    nsExec::ExecToLog 'powershell -NoProfile -Command "if (Get-Process QNote -ErrorAction SilentlyContinue) { exit 1 } else { exit 0 }"'
    Pop $0
    ${If} $0 == 1
      MessageBox MB_OKCANCEL|MB_ICONEXCLAMATION|MB_TOPMOST \
        "$(MsgRunningInstall)" \
        /SD IDOK IDOK kill_it_install IDCANCEL 0
      Abort
      kill_it_install:
        nsExec::ExecToLog 'taskkill /F /IM QNote.exe'
        Pop $0
        Sleep 500
        Goto check_running_install
    ${EndIf}
FunctionEnd
...
Function un.onInit
  ...
  ; 卸载前检测 QNote 是否运行（06-21-installer-packaging）
  ; 用 PowerShell（跨 Windows 语言版本）检测进程：
  ;   exit 1 = 运行中，exit 0 = 未运行
  ; 若运行中弹窗：OK 自动 taskkill 关闭 QNote 后继续，Cancel 终止卸载
  check_running:
    nsExec::ExecToLog 'powershell -NoProfile -Command "if (Get-Process QNote -ErrorAction SilentlyContinue) { exit 1 } else { exit 0 }"'
    Pop $0
    ${If} $0 == 1
      MessageBox MB_OKCANCEL|MB_ICONEXCLAMATION|MB_TOPMOST \
        "$(MsgRunningUninstall)" \
        /SD IDOK IDOK kill_it IDCANCEL 0
      Abort
      kill_it:
        nsExec::ExecToLog 'taskkill /F /IM QNote.exe'
        Pop $0
...
Section "Uninstall"
  ...
  ; 询问是否清理用户数据（06-21-installer-packaging）
  ; 数据存 %APPDATA%\QNote\QNote\（notes.db、settings、logs、CrashDumps、search_index）
  ; 默认保留（IDNO），避免误删；仅当用户明确点"是"才清理
  MessageBox MB_YESNO|MB_ICONQUESTION|MB_TOPMOST \
    "$(MsgAskRemoveUserData)" \
    /SD IDNO IDNO skip_user_data IDYES remove_user_data
  remove_user_data:
    SetShellVarContext current
    RMDir /r "$APPDATA\QNote"
    SetShellVarContext all
  skip_user_data:
